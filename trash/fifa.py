...
from config import APIFOOTBALL_API_KEY
...
today_date = now_pst.strftime("%Y-%m-%d")
tomorrow_date = (now_pst + timedelta(days=1)).strftime("%Y-%m-%d")
yesterday_date = (now_pst - timedelta(days=1)).strftime("%Y-%m-%d")
